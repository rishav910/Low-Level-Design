...
/* 
    Strategy - ShortestDistanceStrategy, LoadBalancingStrategy
    Methods:
...
class LoadBalancingStrategy: public Strategy
{
    ...
    Elevator* getElevator(int floorNum, vector<Elevator*>elevators) override
    {
        int requests = INT_MAX, elevatorNum = 0;
        for(int i=0; i<elevators.size(); i++)
        {
            if(elevators[i]->numberOfRequestsPending() < requests)
            {
                requests = elevators[i]->numberOfRequestsPending();
                elevatorNum = i;
            }
        }
        return elevators[elevatorNum];
...
class ElevatorManager
{
    ...
    void addToQueue(int floor)
    {
        ...
        setStrategy(new LoadBalancingStrategy());
